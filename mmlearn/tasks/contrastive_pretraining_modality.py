...
class ContrastivePretrainingModality(L.LightningModule):
    """Contrastive pretraining task.
    ...
    def forward(self, inputs: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        concatenated_embeddings = torch.cat(
            [embedding for modality, embedding in outputs.items() if modality != Modalities.get_modality("text")],
            dim=-1  # Adjust dim as needed, typically -1 for feature concatenation
        )

        # Add the concatenated embedding as the new modality "rgb"
        outputs[Modalities.get_modality("rgb")] = concatenated_embeddings
    ...
    def _compute_loss(
        ...
        self.new_loss_fn = CLIPLossWithModalityLoss()
        logit_scale = self.postprocessors.norm_and_logit_scale.logit_scale
        
        final_loss = self.new_loss_fn(outputs, logit_scale, self.modality_loss_pairs, embedding_pair_indices)
        ...
        #     contrastive_losses.append(
        #         self.loss_fn(
        #             outputs[Modalities.get_modality("RGB")],
        #             outputs[Modalities.get_modality("RGB")],
        #         )
        #         * loss_pair.weight
        #     )
